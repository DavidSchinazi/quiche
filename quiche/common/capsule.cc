...
quic::QuicSocketAddress CompressionAssignCapsule::kNilAddress = {
    QuicheIpAddress::Any4(), 0};
...
std::string CompressionAssignCapsule::ToString() const {
  return absl::StrCat("COMPRESSION_ASSIGN(context_id=", context_id,
                      ",ip_address=", ip_address_port.ToString(), ")");
}

std::string CompressionCloseCapsule::ToString() const {
  return absl::StrCat("COMPRESSION_CLOSE(context_id=", context_id, ")");
}
...
absl::StatusOr<quiche::QuicheBuffer> SerializeCapsuleWithStatus(
  ...
  switch (capsule.capsule_type()) {
    ...
    case CapsuleType::COMPRESSION_ASSIGN:
      ...
      return capsule.compression_assign_capsule().IpVersion() != 0
                 ? SerializeCapsuleFields(
                       capsule.capsule_type(), allocator,
                       WireVarInt62(
                           capsule.compression_assign_capsule().context_id),
                       WireUint8(
                           capsule.compression_assign_capsule().IpVersion()),
                       WireBytes(
                           capsule.compression_assign_capsule().IpAddress()),
                       WireUint16(capsule.compression_assign_capsule().Port()))
                 : SerializeCapsuleFields(
                       capsule.capsule_type(), allocator,
                       WireVarInt62(
                           capsule.compression_assign_capsule().context_id),
...
absl::StatusOr<Capsule> ParseCapsulePayload(QuicheDataReader& reader,
  ...
  switch (type) {
    ...
    case CapsuleType::COMPRESSION_ASSIGN: {
      ...
      if (address_family == 0) {
        capsule.ip_address_port = CompressionAssignCapsule::kNilAddress;
        return Capsule(std::move(capsule));
      }
      auto address_size = address_family == 4
                              ? QuicheIpAddress::kIPv4AddressSize
                              : QuicheIpAddress::kIPv6AddressSize;
      absl::string_view ip_address_bytes;
      ...
      QUIC_LOG(INFO) << "CompressionAssignCapsule IP: "
                     << ip_address.ToString();
      uint16_t port;
      ...
      QUIC_LOG(INFO) << "CompressionAssignCapsule port: " << port;
      capsule.ip_address_port = quic::QuicSocketAddress(ip_address, port);
      return Capsule(std::move(capsule));
    }
    case CapsuleType::COMPRESSION_CLOSE: {
